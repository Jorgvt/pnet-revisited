...
def main():
    ...
    print(f"Starting JAX PerceptNet (initialized from scratch) optimization experiment on Prop. 6 & 7 using {'WEIGHTED' if args.weighted else 'NON-WEIGHTED'} correlation...")
    ...
    stimuli_flat, plain_flat, slice_sizes = collect_property_stimuli(
        prop6_7, default_prop6_7_config, args.batch_size
    )
    ...
    loss_from_diffs = make_loss_from_diffs(
        prop6_7, default_prop6_7_config, slice_sizes, args.weighted, loss_type=args.loss_type
    )
    ...
    if args.loss_type != "correlation":
        save_path = os.path.join(os.path.dirname(__file__), f"model_pnet_init_prop6_{suffix}_{args.loss_type}.pkl")
    else:
