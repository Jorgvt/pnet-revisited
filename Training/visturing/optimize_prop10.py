...
from visturing.properties import prop10
from visturing.properties.config import default_prop10_config
...
def main():
    ...
    print(f"Starting JAX PerceptNet (initialized from scratch) optimization experiment on Prop. 10 using {'WEIGHTED' if args.weighted else 'NON-WEIGHTED'} correlation...")
    ...
    loss_from_diffs = make_loss_from_diffs(
        prop10, default_prop10_config, slice_sizes, args.weighted, loss_type=args.loss_type
    )
    ...
    if args.loss_type != "correlation":
        save_path = os.path.join(os.path.dirname(__file__), f"model_pnet_init_prop10_{suffix}_{args.loss_type}.pkl")
    else:
        save_path = os.path.join(os.path.dirname(__file__), f"model_pnet_init_prop10_{suffix}.pkl")
    best_corr = -1.0
