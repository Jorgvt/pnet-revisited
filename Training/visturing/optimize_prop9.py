...
from visturing.properties import prop9
from visturing.properties.config import default_prop9_config
...
def main():
    ...
    print(f"Starting JAX PerceptNet (initialized from scratch) optimization experiment on Prop. 9 using {'WEIGHTED' if args.weighted else 'NON-WEIGHTED'} correlation...")
    ...
    stimuli_flat, plain_flat, slice_sizes = collect_property_stimuli(
        prop9, default_prop9_config, args.batch_size
    )
    ...
    loss_from_diffs = make_loss_from_diffs(
        prop9, default_prop9_config, slice_sizes, args.weighted, loss_type=args.loss_type
    )
    ...
    if args.loss_type != "correlation":
        save_path = os.path.join(os.path.dirname(__file__), f"model_pnet_init_prop9_{suffix}_{args.loss_type}.pkl")
    else:
        save_path = os.path.join(os.path.dirname(__file__), f"model_pnet_init_prop9_{suffix}.pkl")
    best_corr = -1.0
